      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            {/* Patient Self-Reports & Pause Window (PART B Integration) */}
            {detail.selfReports && detail.selfReports.length > 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 space-y-3">
                <div className="flex items-center gap-2 text-blue-900 font-bold">
                  <Heart className="h-4 w-4 text-blue-600" />
                  <span>Patient Self-Report</span>
                </div>
                
                {detail.selfReports.map((report: any) => (
                  <div key={report.id} className="bg-white border border-blue-100 rounded-xl p-3 space-y-2">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-blue-700/80 mt-0.5">
                          {new Date(report.reported_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                      {detail.pauseWindowActive && (
                        <span className="text-[9px] font-bold bg-blue-100 text-blue-700 px-2 py-1 rounded-full uppercase tracking-wider whitespace-nowrap">
                          Pause Active
                        </span>
                      )}
                    </div>
                    {report.note && (
                      <p className="text-xs text-blue-800 italic leading-relaxed border-t border-blue-100 pt-2">
                ...
                {detail.pauseWindowActive && (
                  <p className="text-[10px] text-blue-700 italic px-3 py-2 bg-blue-100/50 rounded-lg">
                    ⏸️ Automated struggle alerts are paused until {detail.pauseUntil ? new Date(detail.pauseUntil).toLocaleDateString() : 'unknown date'}
                  </p>
                )}
              </div>
            )}
